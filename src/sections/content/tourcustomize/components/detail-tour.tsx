...
import ChangeDayOrder, { type IDayItem } from "./change-day-order";
...
export const DetailTour = () => {
  ...
  console.log("item detail tour", item);

  const tourName = item?.strServiceName || "Chưa có tên Tour";
  ...
  // 2. Quản lý danh sách ngày tại đây (Source of Truth)
  const [days, setDays] = useState<IDayItem[]>([
    { id: '1', name: 'Ngày 1', date: 'T3, 05 Thg 08, 2025' }
  ]);

  // 3. Thông tin Tour hiện tại (Lấy từ URL ?guid=...)
  const [searchParams] = useSearchParams();
  const tourCustomizedGUID = searchParams.get("guid") || "";
    ...
    <div className={isPopupOpen ? "overflow-hidden h-screen" : ""}>
      ...
      {isEdit ? (
        ...
        <DetailTourContent
          ...
          tourCustomizedGUID={tourCustomizedGUID}
        />
      )}
    </div>
  );
...
const DetailTourHeader = ({ onEdit, isLocked, tourName, tourCode, pax, version }: HeaderProps) => {
  return (
...
interface DetailTourContentProps {
  ...
  onUpdateDays: (days: IDayItem[]) => void;
  isPopupOpen: boolean;
        ...
        {days.map((day, index) => (
          <div key={day.id} className="space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-lg font-bold text-gray-800">{day.name}</h3>
              {index > 0 && <span className="text-xs text-blue-500 font-medium italic">{day.date}</span>}
            </div>
            <ListTour
